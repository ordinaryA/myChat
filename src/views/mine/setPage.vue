<template>
    <div class="com_layout setPage">
        ...
        >
            ...
            >
                <mu-list-item-content :class="item.center?'text_center':null">{{item.tag}}</mu-list-item-content>
        ...
        <mu-bottom-sheet :open.sync="open">
            <mu-list
                @item-click="closeBottomSheet"
                class="exit_list"
            >
                <mu-sub-header class="exit_head">退出后不会删除任何历史数据，下次登录依然可以使用本账号。</mu-sub-header>
                <mu-list-item
                    button
                    v-for="(i,idx) in exit"
                    :key="idx"
                    ...
                    @click="exitFn(idx)"
                >
                    <mu-list-item-title :class="i.className">{{i.text}}</mu-list-item-title>
                </mu-list-item>
            </mu-list>
        </mu-bottom-sheet>
    </div>
