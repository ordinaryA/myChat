<template>
  <div>
    <div class="frd_first">
      ...
      <div class="ava_b cl">
        <mu-avatar class="ava_this fl" size="60">
    ...
    <mu-button flat class="border_box frd_o">更多信息</mu-button>
    <mu-button
    ...
    <mu-button
      ...
      @click="makeFriend"
    >
      <svg class="icon" aria-hidden="true" style="margin:-5px 5px 0 0">
...
export default {
    ...
    makeFriend() {
      ...
      this.$confirm("您要添加他为好友吗", "提示").then(async ({ result }) => {
        if (result) {
          ...
          const { data } = await _http.makeFriend({
            applyUser,
            receiveUser: friendId
          });
          const { code, msg } = data;
          if (code === 1) {
  ...
  computed: {
    ...
    beFriendsTime() {
      ...
      return beTime
        ? "你们已成为好友" + otherDate(Number(new Date()) - beTime)
        : null;
    }
  }
};
