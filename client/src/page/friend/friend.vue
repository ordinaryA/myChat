<template>
  <div class="com_layout">
    <mu-appbar class="com_header">
      ...
      <mu-button icon slot="right" @click="openAddFn" v-else>
        <mu-icon value=":iconfont icon-friendadd" />
      </mu-button>
    </mu-appbar>
    <mu-list class="border_box1" v-if="sendApply.length > 0">
      <div class="frd_title">已发送的好友申请</div>
      <div v-for="(i, idx) in sendApply" :key="idx">
        <mu-list-item avatar button @click="checkInfo(i, 'apply')">
          <mu-list-item-action>
            <mu-avatar>
    ...
    <mu-list class="border_box1 my_frd_list" v-if="friendList.length > 0">
      <div class="frd_title">好友列表</div>
      <div v-for="(i, idx) in friendList" :key="idx">
        <mu-list-item avatar button @click="checkInfo(i, 'friends')">
          <mu-list-item-action>
            <mu-avatar>
              <img :src="`./static/avatar/${i.avatar}`" />
            </mu-avatar>
    ...
    <div v-else class="frd_tip">暂无好友，点击右上角可添加好友 φ(゜▽゜*)♪</div>
    <mu-dialog
      width="360"
    ...
    >
      <div class="ava_exsit">
        <mu-button slot="left" icon @click="closeAddFn">
      ...
      <div class="ad_con">
        <div class="ad_search">
          <input
            ...
            v-model="friendFlag"
          />
          <svg class="icon" aria-hidden="true" @click="addFriend">
            <use xlink:href="#icon-huabanfuben1"></use>
          </svg>
        </div>
        <div class="border_box1" v-if="showSearchResult">
          <mu-list v-if="searchUserList.length > 0">
            <div v-for="(i, idx) in searchUserList" :key="idx">
              <mu-list-item avatar button @click="checkInfo(i, 'search')">
                <mu-list-item-action>
                  <mu-avatar>
                    <img :src="`./static/avatar/${i.avatar}`" />
                  </mu-avatar>
          ...
          <div v-else class="hasnotUser">该用户不存在</div>
        </div>
        <div class="frd_apply">
          <div class="apply_title" v-if="friendApply.length > 0">好友申请</div>
          <div
            class="apply_title"
            v-else
          ...
          <div class="apply_list">
            <mu-expansion-panel
              ...
              :key="idx"
            >
              <div slot="header" class="apply_header">
...
export default {
  ...
  methods: {
    ...
    async disposeApply(type, applyId) {
      const { data } = await _http.disposeApply({
        type,
        selfId: GET_SESSION("USERID"),
        applyId
      });
      const { code, msg } = data;
      if (code === 1) {
